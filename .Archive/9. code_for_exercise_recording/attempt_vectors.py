...
for i in range(0, 119):
    with open(f'all_probes2/{i}_probe/accelx_data.txt', 'r') as f:
        ax = np.array([float(x) for x in f.read().split('\n')[:-1]])
    with open(f'all_probes2/{i}_probe/accely_data.txt', 'r') as f:
        ay = np.array([float(x) for x in f.read().split('\n')[:-1]])
    with open(f'all_probes2/{i}_probe/accelz_data.txt', 'r') as f:
        az = np.array([float(x) for x in f.read().split('\n')[:-1]])

    with open(f'all_probes2/{i}_probe/gyrox_data.txt', 'r') as f:
...
csvfile = open(f'all_probes2/dataset.csv', 'w', newline='\n')
spamwriter = csv.writer(csvfile, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
for i in range(len(vectors)):
    with open(f'all_probes2/{i+1}_probe/Result', 'r') as f:
        success = int(f.read())
