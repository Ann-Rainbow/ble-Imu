import csv
with open("all_probes/320_probe/accelx_data.txt", 'r') as f:
    # print(f.read())
    ax = [float(x) for x in f.read().split('\n')[:-1]]
# print(ax)
with open('all_probes/320_probe/accely_data.txt', 'r') as f:
    ay = [float(x) for x in f.read().split('\n')[:-1]]
with open('all_probes/320_probe/accelz_data.txt', 'r') as f:
    az = [float(x) for x in f.read().split('\n')[:-1]]

with open('all_probes/320_probe/gyrox_data.txt', 'r') as f:
...
csvfile = open('all_probes/320_probe/trololo_probe.csv', 'w', newline='')
